...
use:
    - <vec3> transparent(<Material> mat, <ShadingData> shadingData)
license:
...
vec3 transparent(Material mat, ShadingData shadingData) {
    ...
    vec3 view   = shadingData.V;
    vec3 eta    = ior2eta(mat.ior);
    vec3 Fr     = shadingData.indirectSpecular;
    vec3 T      = max(vec3(0.0, 0.0, 0.0), 1.0-Fr);
    float roughness = shadingData.roughness;

    #if defined(TRANSPARENT_DISPERSION) && defined(TRANSPARENT_DISPERSION_PASSES)
        float pass_step = 1.0/float(TRANSPARENT_DISPERSION_PASSES);
        vec3 bck = vec3(0.0, 0.0, 0.0);
